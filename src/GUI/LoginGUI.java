...
public class LoginGUI {
    private JFrame frBackground = new JFrame("QUẢN LÝ BÁN VÉ MÁY BAY");
    ...
    public void initComponents() {
        ...
        ImageIcon imageIcon = new ImageIcon(new ImageIcon(getClass().getResource("/GUI/asset/new.png")).getImage().getScaledInstance(550, 400, Image.SCALE_SMOOTH));
        ...
        ImageIcon iconAdd = new ImageIcon(new ImageIcon(getClass().getResource("/GUI/asset/hien.jpg")).getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH));
        ...
        btnAnPass.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (echoChar == 0) {
                    txtPass.setEchoChar('•');
                     iconAdd = new ImageIcon(new ImageIcon(getClass().getResource("/GUI/asset/hien.jpg")).getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH));
                    btnAnPass.setIcon(iconAdd);
                } else {
                    txtPass.setEchoChar((char) 0);
                     iconAdd = new ImageIcon(new ImageIcon(getClass().getResource("/GUI/asset/an.jpg")).getImage().getScaledInstance(40, 40, Image.SCALE_SMOOTH));
                    btnAnPass.setIcon(iconAdd);
                }
            }
        });
    ...
    public boolean checkTK(ArrayList<NhanVienDTO> list, String tk) {
        for(NhanVienDTO s:list) 
            if(s.getMaNV().trim().toLowerCase().equals(tk.trim().toLowerCase())) return true;
        return false;
    }
    ...
    public boolean checkMK(ArrayList<NhanVienDTO> list, String tk,String mk) {
        for(NhanVienDTO s:list) 
            if(s.getMaNV().trim().toLowerCase().equals(tk.toLowerCase().trim()) && s.getmKNV().equals(mk)) {
    ...
    //check chức vụ
    public boolean checkRole(ArrayList<NhanVienDTO> list, String tk) {
        for(NhanVienDTO s:list) 
            if(s.getChucVu().equals("Quản Lý") && s.getMaNV().trim().toLowerCase().equals(tk.trim().toLowerCase())){
                mainGUI.nameNV=s.getTenNV();
                 return true;
            }
        return false;
        ...
        if(checkNull()) {
            String pass = new String(txtPass.getPassword());
                ...
                if(checkTK(list, txtUser.getText())==false)
                    JOptionPane.showMessageDialog(frBackground, "Tài khoản không tồn tại", "Thông báo", JOptionPane.ERROR_MESSAGE);
                else if(checkMK(list, txtUser.getText(), pass)){
                    if(checkRole(list,txtUser.getText())) {
                        frBackground.setVisible(false);
                        mainGUI.taiKhoan = txtUser.getText().toUpperCase().trim();
                        new AdminGUI();
                    } else {
                        frBackground.setVisible(false);
                        mainGUI.taiKhoan = txtUser.getText().toUpperCase().trim();
                   ...
                   JOptionPane.showMessageDialog(frBackground, "Sai mật khẩu", "Thông báo", JOptionPane.ERROR_MESSAGE);
        }
    }
